...
- **可选 Git worktree 隔离** — 在 Git 项目中可让新 Agent 在独立 checkout 工作；子 Agent 沿用父 Agent 的 checkout，默认仍使用共享项目目录。
- **Worktree 审查面板** — 可查看 Diff、接受或拒绝改动。接受前会拒绝脏工作区并预检冲突；通过后以 squash merge 写回。冲突只列出文件、不强制覆盖，拒绝会清理临时 worktree 和分支。
- **工具权限策略** — 支持 `ask`、`edits`、`allow` 三档会话策略，可在新会话预设或运行时切换；允许/拒绝请求支持队列、超时和响应丢失时均按拒绝处理，委派 Agent 继承父策略，无法读取时回退到 `ask`。

### 审查、撤回与运行反馈

- 每轮回复展示改动文件、增删行与 Diff 入口；支持撤回单个文件、整轮改动或一个 Agent 的全部改动。批量撤回按逆序执行，且不会覆盖之后发生的文件修改。
- 多个共享目录 Agent 修改同一文件时显示冲突提示；`turn_timeout` 等运行时通知会写入对话记录，避免误判为崩溃。
- 文件审查与旁路提问拆分为独立入口，交互更清晰。

### 模型与 Provider 配置

- 设置页和模型选择器统一使用 Nova 的 JSON 模型目录，并在窗口重新获得焦点时刷新。
- 支持模型查看、编辑、删除和重命名；重命名会替换旧记录而非产生重复项，删除模型不会删除 Provider 或凭据。
- 新增未知/自定义 Provider 入口及 Provider Logo，简化模型表单并移除易误解的最大输出 Token 手工字段。

### 界面与构建

- 改进项目选择器滚动、照片背景下的深色材质、危险操作按钮与活动热度样式。
- 修复 Nova 1.5 移除 interactive assets 后的 sidecar 打包：不再引用空资源 glob，只捆绑运行所需的 `theme/` 和 `export-html/`。
